...
module buffer_net #(parameter N =`numb)(
   ...
   output logic signed [7:0]a_row [0:N-1],
   output logic signed [7:0]b_col [0:N-1],
   output logic             valid_out [0:N-1],
   output logic             load
    );

    logic signed [7:0]         a_reg [0:N-1][0:N-1];
    logic [$clog2(N*N)-1:0]    a_cnt;
    logic                      a_loaded;
    
    logic signed [7:0]         b_reg [0:N-1][0:N-1];
    ...
    always_ff @(posedge clk)begin
        if(!rst_n)begin
            ...
            a_reg[a_cnt/N][a_cnt%N] <= a_in;
            if(a_cnt == N*N-1)begin
                a_cnt <= 0;
                a_loaded <= 1'b1;
            end else 
                a_cnt <= a_cnt + 1;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            b_reg[b_cnt%N][b_cnt/N] <= b_in;
            if (b_cnt == N*N-1) begin
                b_cnt    <= '0;
                b_loaded <= 1'b1;
            end else
                b_cnt <= b_cnt + 1;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            end else if(a_loaded && b_loaded && !compute_started)begin
